...
def promote_types(args):
    ...
    for arg in args:
        if isinstance(arg, str):
            continue
        elif isinstance(arg, OpsValue) and isinstance(arg.value, CSEVariable) and arg.value.dtype is not None:
            dtype_prop_candidates.append(arg.value)
        elif (
            (isinstance(arg, CSEVariable) and arg.dtype is not None)
            or isinstance(arg, torch._prims_common.Number)
        ):
